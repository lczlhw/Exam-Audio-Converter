...
def process_single(file, input_path, output_path, trim_start, trim_end, gain, fmt):
    input_file = (input_path / file).resolve()
    output_file = (output_path / f"{file.stem}_processed.{fmt}").resolve()
    return process_audio(input_file, output_file, trim_start, trim_end, gain, fmt)

class Worker:
    def __init__(self, input_path, output_path, trim_start, trim_end, gain, fmt):
